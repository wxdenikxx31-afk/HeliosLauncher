...
async function populateServerListings(){
    ...
    // Server info card - use the first server for common info
    const firstServ = servers[0]
    const serverInfoHtml = `
        <div class="serverInfoAddress">${firstServ.rawServer.address}</div>
        <div class="serverInfoVersion">Minecraft ${firstServ.rawServer.minecraftVersion}</div>
    `
    document.getElementById('serverInfoCard').innerHTML = serverInfoHtml

    // Modpack listings
    let htmlString = ''
    for(const serv of servers){
        const isFull = serv.rawServer.id.toLowerCase().includes('full')
        const modpackIcon = isFull ? '⚡' : '🍃'
        const modpackTag = isFull ? 'Шейдеры + Ресурспаки' : 'Без шейдеров'

        htmlString += `<button class="serverListing modpackCard" servid="${serv.rawServer.id}" ${serv.rawServer.id === giaSel ? 'selected' : ''}>
            <div class="modpackIcon">${modpackIcon}</div>
            <div class="modpackDetails">
                <span class="modpackName">${serv.rawServer.name}</span>
                <span class="modpackTag">${modpackTag}</span>
                <div class="modpackInfo">
                    <div class="serverListingVersion">${serv.rawServer.minecraftVersion}</div>
                    <div class="serverListingRevision">v${serv.rawServer.version}</div>
                </div>
            </div>
        </button>`
    }
